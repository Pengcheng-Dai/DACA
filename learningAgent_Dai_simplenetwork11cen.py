                            ...
                            for a in range(self.action_num):
                                ...
                                for j in range(self.agent_num):
                                    if j is not i:
                                        term2 += self.local_objective(j, global_state)
                                # print("i", i)
                                # print("policygradient", (prob_vec[a] * term1 * term2)/(1 - self.gamma))
...
#     np.random.seed(t_seed)
    ...
    network.add_edge((0, 1)) # network很简单
...
#####################################################################################
    ...
    for m in trange(T):
        ...
        # print("policygradient", gradients)
    ...
    # # 保存数据
    # np.save("./simple_network/objective_perform_cen.npy", objective_perform)
    # np.save("./simple_network11/objective_perform_cen{}.npy".format(1000), objective_perform)
    #
    # # # 画图
    # plt.figure()
    # plt.plot(objective_perform)
    # plt.savefig("./simple_network11/one_bridge_cen.pdf")
    # plt.savefig("./simple_network11/one_bridge_cen.png")
    # 输出结束时间
    formatted_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
